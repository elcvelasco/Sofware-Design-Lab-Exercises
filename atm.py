 ...
 *
 ...
 program description: 
                        An ATM program allows a user an indefinite number of attempts to log in. 
                         Fix the program so that it displays a popup message that the police will be called 
                         after a user had three successive failures consecutively. 
                         The program should disable the login button and a pop up message will be shown 
                         when this happens.  
 *
"""
...
class ATM(EasyFrame):
    ...
    def login(self):
        ...
        if self.account:
            ...
            if self.count >= 3:
                self.messageBox(title="INVALID ACCOUNT", message="3 attempts has been reached. The police will be called immediately.")
                self.loginButton = self.addButton(row = 3, column = 2,
                                          text = "Login",
                                          state= "disabled")
